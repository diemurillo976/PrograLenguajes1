...
void addOnlineUser(struct infoCard* info, struct sockaddr_in* address){
	...
	for (i = 0; i < 100; i++){
		if((*(users+i)).online == 0){
			...
			strcpy((*(users+i)).info.userColor, (*info).userColor);
...
void printOnlineUsers(){
	...
	for (i = 0; i < 100; i++){
		if((*(users+i)).online == 1){
			printf("user %s%s\033[0m \n",(*(users+i)).info.userColor,(*(users+i)).info.userId);
		}
	}
}
...
int main(int argc, char *argv[])
{
	...
	printf("\033[91m==[ Hostname: %s ]==\033[0m \n", hostname);
	...
	while(1)
	{
		...
		strcpy(myInfo.userColor, myColor);
...
void standByMe(struct infoCard *myInfo, int sockUDP){
	...
	char buffer[256];
	int success;
	printf("Welcome: %s%s\033[0m \n", myInfo->userColor, myInfo->userId);
	short breakUpFlag = 0;
